...
# Code based on: https://github.com/Orion-AI-Lab/S4A-Models/blob/master/coco_data_split.py (Sen4AgriNet Experiments)


full_patch_dataset = pd.read_pickle("Dataframes/Patch_Data_Frame.pkl") # Read in Dataframe

def filter_dataframe_n_retieve_unique_labels(tiles, 
                                             years, 
                                             common_labels=None): # Filter Dataframe based on selected Tiles and Years whilst retrieving Unique Labels per Patch, 
...
def train_val_test_split(experiment=2, 
                         ...
                         n_folds=3,
                         random_state=42): # Create Stratified Train/Val/Test Split for a specified Size and a specified Experiment for a number of Folds
    train_tiles, train_years, test_tiles, test_years, common_lbls = None, None, None, None, None

    if not os.path.exists(f"Dataframes/{downsampling_percentage}%/"): # Create Path to Save Dataframes of each Subset to
        os.mkdir(f"Dataframes/{downsampling_percentage}%/")
    
    if not os.path.exists(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}"):
        os.mkdir(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}")

    for k in range(n_folds):
        if not os.path.exists(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}/Fold_{k+1}"):
            os.mkdir(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}/Fold_{k+1}")

    if experiment == 2: # Select required Years and Tiles depending on Experiment
    ...
    if downsampling_percentage != 100: # Perform Stratified Sampling for N-Fold Training and Validation Set
    ...
    for k in range(n_folds): # Perform Stratified Sampling for N-Fold Test Set and Save the various Subsets
        ...
        X_train.to_csv(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}/Fold_{k+1}/Training_Set.csv", index=False)
        X_val.to_csv(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}/Fold_{k+1}/Validation_Set.csv", index=False)
        X_test.to_csv(f"Dataframes/{downsampling_percentage}%/Experiment_{experiment}/Fold_{k+1}/Test_Set.csv", index=False)

for dp in [5, 10, 25, 50, 75, 100]: # Perform Data Frame Transformation for various sizes
