...
dataset_path = r'D:\UC Project Sen4AgriNet Dataset' # Specify Path to Dataset

if not os.path.exists("Experiments_Dataframes\Patch_Data_Frame_2.pkl"): # Create and Save Dataframe to File in case it doesn't exist
    pd.DataFrame(columns=['Year',
                          'Country', 
                          'Tile', 
                          'Patch',
                          'Labels']).to_pickle("Experiments_Dataframes\Patch_Data_Frame_2.pkl") 

patch_df = pd.read_pickle("Experiments_Dataframes\Patch_Data_Frame_2.pkl") # Read in current Dataframe
...
if current_length != len(patch_df):
    patch_df.to_pickle("Experiments_Dataframes\Patch_Data_Frame_2.pkl")
